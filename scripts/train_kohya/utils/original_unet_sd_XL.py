...
class SdxlUNet2DConditionModel(nn.Module):
    ...
    # endregion
    def forward(self, x, timesteps=None, encoder_hidden_states=None, added_cond_kwargs=None, **kwargs):
        ...
        y = torch.cat([added_cond_kwargs["text_embeds"], added_cond_kwargs["time_ids"]], dim=1).to(x.dtype)
        ...
        for module in self.input_blocks:
            h = call_module(module, h, emb, encoder_hidden_states)
            hs.append(h)

        h = call_module(self.middle_block, h, emb, encoder_hidden_states)
        ...
        h = h.type(x.dtype)
        h = call_module(self.out, h, emb, encoder_hidden_states)
