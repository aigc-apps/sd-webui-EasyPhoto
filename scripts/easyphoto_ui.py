...
import modules
from scripts.easyphoto_config import cache_log_file_path, models_path, user_id_outpath_samples, easyphoto_outpath_samples
    ...
    with gr.Blocks(analytics_enabled=False) as easyphoto_tabs:
            ...
            with gr.Blocks():
                with gr.Row():
                    ...
                    with gr.Column():
                        ...
                        main_image = gr.Image(
                            label="Main Image", elem_id="{id_part}_image", show_label=False, source="upload", type="filepath"
                        )
                    ...
                    with gr.Column():
                        ...
                        with gr.Accordion("Advanced Options", open=True):
                            ...
                            with gr.Row():
                                ...
                                gradient_accumulation_steps = gr.Textbox(label="gradient accumulationsteps", value=4, interactive=True)
                            ...
                            with gr.Row():
                                refine_mask = gr.Checkbox(label="Refine Mask", value=True)
                                use_mask = gr.Checkbox(label="Use Mask", value=True)

                            with gr.Row():
                                validation = gr.Checkbox(label="Validation", value=False, visible=False)
                                enable_rl = gr.Checkbox(label="Enable RL (Reinforcement Learning)", value=False, visible=False)
        ...
        with gr.TabItem("Inference"):
            ...
            with gr.Blocks() as demo:
                    ...
                    with gr.Column():
                        ...
                        with gr.Column() as infer_lora_option:
                            ...
                            with gr.Row():
                                ...
                                refresh.click(fn=select_function, inputs=[], outputs=[uuid])

                            with gr.Row():
                                match_and_paste = gr.Checkbox(label="Match and Paste", value=True)

                                remove_target = gr.Checkbox(label="Remove Target", value=False)
                            ...
                            with gr.Accordion("Advanced Options", open=False):
                                ...
                                with gr.Row():
                                    first_diffusion_steps = gr.Slider(minimum=15, maximum=50, value=50, step=1, label="Diffusion steps")
                                    first_denoising_strength = gr.Slider(
                                        minimum=0.40, maximum=1.00, value=0.70, step=0.05, label="Diffusion denoising strength"
                                    )

                                with gr.Row():
                                    lora_weight = gr.Slider(minimum=0, maximum=1, value=0.8, step=0.1, label="LoRA weight")
                                    iou_threshold = gr.Slider(minimum=0, maximum=1, value=0.7, step=0.05, label="IoU Threshold ")

                                with gr.Row():
                                    angle = gr.Slider(minimum=-90, maximum=90, value=0.0, step=1, label="Angle")
                                    azimuth = gr.Slider(minimum=-60, maximum=60, value=0.0, step=1, label="Azimuth")
                                    ratio = gr.Slider(minimum=0.5, maximum=5.5, value=1.0, step=0.1, label="Ratio")

                                with gr.Row():
                                    batch_size = gr.Slider(minimum=1, maximum=10, value=1, step=1, label="Batch Size")
                                ...
                                with gr.Row():
                                    ...
                                    pure_image = gr.Checkbox(label="Pure Image", value=True)
                                    global_inpaint = gr.Checkbox(label="Global Inpaint", value=False)
                        ...
                        with gr.Column(visible=False) as infer_anydoor_option:
                            enhance_with_lora = gr.Checkbox(label="Enhance with LoRA", value=False)
                        ...
                        enhance_with_lora.change(lambda x: gr.update(visible=x), inputs=[enhance_with_lora], outputs=[infer_lora_option])

                        infer_way.change(
                            fn=infer_way_change,
                            inputs=[infer_way, enhance_with_lora],
                            outputs=[infer_way, infer_lora_option, infer_anydoor_option],
                        )

                    with gr.Column():
                        gr.Markdown("Generated Results")

                        output_images = gr.Gallery(label="Output", show_label=False).style(
                        ...
                        with gr.Row():
                            ...
                            buttons = {
                                ...
                                "extras": ToolButton_webui(
                                    "📐", elem_id=f"{tabname}_send_to_extras", tooltip="Send image and generation parameters to extras tab."
                                ),
                            }
                        ...
                        for paste_tabname, paste_button in buttons.items():
                            parameters_copypaste.register_paste_params_button(
                                parameters_copypaste.ParamBinding(
                                    ...
                                    source_image_component=output_images,
                                    paste_field_names=[],
                                )
                            )
                ...
                uuid.change(fn=show_ref_image, inputs=[uuid], outputs=[infer_progress, reference_image, show_default_mask])
                ...
                show_default_mask.change(fn=show_ref_mask, inputs=[uuid, show_default_mask], outputs=[infer_progress, reference_mask])
