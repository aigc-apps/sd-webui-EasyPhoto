...
def upload_file(files, current_files):
    file_paths = [file_d['name']
                  for file_d in current_files] + [file.name for file in files]
    return file_paths
            ...
            for _checkpoint in files:
                if _checkpoint.endswith(("pth", "safetensors", "ckpt")):
                    rel_path = os.path.relpath(
                        os.path.join(root, _checkpoint), models_dir)
    ...
    with gr.Blocks(analytics_enabled=False) as easyphoto_tabs:
        with gr.Tabs(elem_id="mode_easyphoto"):
            with gr.TabItem('Train'):
                ...
                with gr.Blocks():
                    with gr.Row():
                        uuid = gr.Text(label="User_ID",
                                       value="", visible=False)

                        with gr.Column():
                            gr.Markdown('Training photos')

                            instance_images = gr.Gallery().style(
                            ...
                            clear_button.click(
                                fn=lambda: [], inputs=None, outputs=instance_images)

                            upload_button.upload(upload_file, inputs=[
                                                 upload_button, instance_images], outputs=instance_images, queue=False)
                        ...
                        with gr.Column():
                            ...
                            with gr.Accordion("Advanced Options", open=True):
                                with gr.Row():
                                    train_mode_choose = gr.Dropdown(value="Train Human Lora", elem_id='radio', scale=1, min_width=20, choices=[
                                                                    "Train Human Lora", "Train Scene Lora"], label="The Type of Lora", visible=True)
                                    sd_model_checkpoint = gr.Dropdown(value="Chilloutmix-Ni-pruned-fp16-fix.safetensors", scale=3, choices=list(set(
                                        ["Chilloutmix-Ni-pruned-fp16-fix.safetensors"] + checkpoints + external_checkpoints)), label="The base checkpoint you use.", visible=True)

                                    checkpoint_refresh = ToolButton(
                                        value="\U0001f504")
                                ...
                                with gr.Row(visible=False) as sdxl_row:
                                    ...
                                    sdxl_training_note = gr.Markdown(
                                        value="**Please check the [[wiki]]({}) before SDXL training**.".format(
                                            sdxl_wiki_url),
                                        visible=True
                                    )
                                ...
                                enable_rl.change(lambda x: rl_option_row1.update(
                                    visible=x), inputs=[enable_rl], outputs=[rl_option_row1])
                                enable_rl.change(lambda x: rl_option_row1.update(
                                    visible=x), inputs=[enable_rl], outputs=[rl_notes])

                                # We will update the default training parameters by the checkpoint type.
                                def update_train_parameters(sd_model_checkpoint):
                                    checkpoint_type = get_checkpoint_type(
                                        sd_model_checkpoint)
                                ...
                                sd_model_checkpoint.change(
                                    ...
                                    outputs=[
                                        sdxl_row, resolution, max_train_steps, rank, network_alpha, validation]
                                )
                            ...
                            train_mode_choose.change(update_train_mode, inputs=train_mode_choose, outputs=[resolution, rank, network_alpha, crop_ratio, training_prefix_prompt_row, val_and_checkpointing_steps,
                                                     validation, enable_rl, rl_option_row1, rl_notes, skin_retouching_bool, human_train_notes, scene_train_notes, human_upload_note, scene_upload_note])
                    ...
                    with gr.Box():
                        logs_out = gr.Chatbot(
                            label='Training Logs', height=200)
                        block = gr.Blocks()
                        with block:
                            block.load(refresh_display, None,
                                       logs_out, every=3)
            ...
            with gr.TabItem('Photo Inference', elem_id="tab_photo_inference"):
                dummy_component = gr.Label(visible=False)
                training_templates = glob.glob(os.path.join(os.path.abspath(os.path.dirname(
                    __file__)).replace("scripts", "models"), 'training_templates/*.jpg'))
                infer_templates = glob.glob(os.path.join(os.path.abspath(os.path.dirname(
                    __file__)).replace("scripts", "models"), 'infer_templates/*.jpg'))
                preset_template = list(
                    training_templates) + list(infer_templates)
                ...
                with gr.Blocks() as demo:
                    with gr.Row():
                        with gr.Column():
                            with gr.Tabs(elem_id="mode_easyphoto_photo_inference"):
                                with gr.TabItem("Photo2Photo") as template_images_tab:
                                    upload_way = gr.Radio(["Template Gallery", "Single Image Upload", "Batch Images Upload"],
                                                          value="Template Gallery", show_label=False, elem_id='mode_photo_to_photo')

                                    with gr.Column(elem_id="mode_tempalte_gallery") as template_gallery:
                                        template_gallery_list = [
                                            (i, i) for i in preset_template]
                                        ...
                                        selected_template_images = gr.Text(
                                            show_label=False, visible=False, placeholder="Selected")
                                        gallery.select(
                                            select_function, None, selected_template_images)

                                    with gr.Column(elem_id="mode_single_image_upload", visible=False) as single_image_upload:
                                        init_image_photo_inference = gr.Image(
                                            label="Image for skybox", elem_id="{id_part}_image", show_label=False, source="upload")

                                    with gr.Column(elem_id="mode_batch_images_upload", visible=False) as batch_images_upload:
                                        ...
                                        with gr.Row():
                                            ...
                                            clear_dir_button = gr.Button(
                                                "Clear Photos")
                                        clear_dir_button.click(
                                            fn=lambda: [], inputs=None, outputs=uploaded_template_images)

                                        upload_dir_button.upload(upload_file, inputs=[
                                                                 upload_dir_button, uploaded_template_images], outputs=uploaded_template_images, queue=False)
                                    ...
                                    upload_way.change(upload_way_change, upload_way, [
                                                      template_gallery, single_image_upload, batch_images_upload])
                                ...
                                with gr.TabItem("Text2Photo") as text2photo_tab:
                                    with gr.Column():
                                        ...
                                        with gr.Accordion("Scene Lora (Click here to select Scene Lora)", open=False):
                                            with gr.Column():
                                                ...
                                                def scene_refresh_function():
                                                    scene = [[os.path.join(os.path.abspath(os.path.dirname(__file__)).replace(
                                                        "scripts", "images"), "scene_lora", f'{_scene}.jpg'), _scene] for _scene in DEFAULT_SCENE_LORA]
                                                    _scenes = os.listdir(
                                                    ...
                                                    for _scene in _scenes:
                                                        if check_scene_valid(_scene, models_path):
                                                            ...
                                                            if not os.path.exists(ref_image):
                                                                ref_image = os.path.join(os.path.abspath(os.path.dirname(
                                                                    __file__)).replace("scripts", "images"), 'no_found_image.jpg')
                                                            scene.append(
                                                                [ref_image, os.path.splitext(_scene)[0]])
                                                    scene = sorted(scene)
                                                    scene.insert(0, [os.path.join(os.path.abspath(os.path.dirname(__file__)).replace(
                                                        "scripts", "images"), 'no_found_image.jpg'), "none"])
                                                ...
                                                autodownload_notebook = gr.Markdown(
                                                    value="**The preset Lora will be downloaded on the first click.**", show_label=False, visible=True)
                                                with gr.Row():
                                                    ...
                                                    scene_id_gallery.select(scene_change_function, [scene_id_gallery], [
                                                                            text_to_image_input_prompt, scene_id])

                                                    scene_id_refresh = ToolButton(
                                                        value="\U0001f504")
                                                    scene_id_refresh.click(fn=scene_refresh_function, inputs=[
                                                    ], outputs=[scene_id_gallery])
                                        ...
                                        with gr.Row():
                                            prompt_generate_sd_model_checkpoint = gr.Dropdown(value="LZ-16K+Optics.safetensors", choices=list(set(
                                                ["Chilloutmix-Ni-pruned-fp16-fix.safetensors"] + checkpoints + external_checkpoints)), label="The checkpoint you use for text to image prompt generate.", interactive=True, visible=True)

                                            prompt_generate_checkpoint_refresh = ToolButton(
                                                value="\U0001f504")
                            ...
                            model_selected_tabs = [
                                template_images_tab, text2photo_tab]
                            for i, tab in enumerate(model_selected_tabs):
                                tab.select(fn=lambda tabnum=i: tabnum, inputs=[
                                ], outputs=[model_selected_tab])
                                tab.select(generate_tabs, [
                                           model_selected_tab, upload_way], state_tab)

                            upload_way.change(
                                generate_tabs, [model_selected_tab, upload_way], state_tab)

                            with gr.Row():
                                sd_model_checkpoint = gr.Dropdown(value="Chilloutmix-Ni-pruned-fp16-fix.safetensors", choices=list(set(
                                    ["Chilloutmix-Ni-pruned-fp16-fix.safetensors"] + checkpoints + external_checkpoints)), label="The base checkpoint you use.", interactive=True, visible=True)
                            ...
                            with gr.Row():
                                ...
                                sd_model_checkpoint.change(
                                    fn=update_infer_note, inputs=sd_model_checkpoint, outputs=[infer_note])
                            ...
                            with gr.Row():
                                ...
                                num_of_faceid = gr.Dropdown(value=str(1), elem_id='dropdown', choices=[
                                                            1, 2, 3, 4, 5], label=f"Num of Faceid")

                                uuids = []
                                visibles = [True, False, False, False, False]
                                for i in range(int(5)):
                                    uuid = gr.Dropdown(value="none", elem_id='dropdown', choices=[
                                                       "none"] + ids, min_width=80, label=f"User_{i} id", visible=visibles[i])
                                    uuids.append(uuid)
                                ...
                                num_of_faceid.change(update_uuids, inputs=[
                                                     num_of_faceid], outputs=uuids)
                            ...
                            with gr.Accordion("Advanced Options", open=False):
                                ...
                                with gr.Row():
                                    ...
                                    super_resolution.change(lambda x: super_resolution_method.update(
                                        visible=x), inputs=[super_resolution], outputs=[super_resolution_method])
                                    background_restore.change(lambda x: background_restore_denoising_strength.update(
                                        visible=x), inputs=[background_restore], outputs=[background_restore_denoising_strength])
                                    makeup_transfer.change(lambda x: makeup_transfer_ratio.update(
                                        visible=x), inputs=[makeup_transfer], outputs=[makeup_transfer_ratio])
                                    ip_adapter_control.change(lambda x: ip_adapter_weight.update(
                                        visible=x), inputs=[ip_adapter_control], outputs=[ip_adapter_weight])
                            ...
                            with gr.Row():
                                ipa_image_path = gr.Image(
                                    label="Image Prompt for IP-Adapter Control", show_label=True, source="upload", type="filepath", visible=False)
                            ip_adapter_control.change(lambda x: ipa_image_path.update(visible=x), inputs=[
                                                      ip_adapter_control], outputs=[ipa_image_path])
                            ip_adapter_control.change(lambda x: ipa_note.update(visible=x), inputs=[
                                                      ip_adapter_control], outputs=[ipa_note])
                        ...
                        with gr.Column():
                            ...
                            photo_infer_output_images = gr.Gallery(
                                label='Output',
                                show_label=False
                            ...
                            selected_photo_infer_output_image = gr.Text(
                                show_label=False, visible=False, placeholder="Selected")
                            photo_infer_output_images.select(
                                select_function, photo_infer_output_images, selected_photo_infer_output_image)
                            ...
                            with gr.Row():
                                ...
                                buttons_photo_infer = {
                                    ...
                                    'tryon': ToolButton_webui('👔', elem_id=f'{tabname}_send_to_tryon', tooltip="Send image and generation parameters to tryon tab.")
                                }

                            face_id_text = gr.Markdown(
                                "Face Similarity Scores", visible=False)
                            ...
                            display_score.change(lambda x: face_id_text.update(visible=x), inputs=[
                                                 display_score], outputs=[face_id_text])
                            display_score.change(lambda x: face_id_outputs.update(visible=x), inputs=[
                                                 display_score], outputs=[face_id_outputs])
                    ...
                    display_button.click(
                        ...
                        inputs=[
                            sd_model_checkpoint, selected_template_images, init_image_photo_inference, uploaded_template_images,
                        ...
                        outputs=[infer_progress,
                                 photo_infer_output_images, face_id_outputs]
                    )
                ...
                if not video_visible:
                    ...
                    with gr.Blocks() as demo:
                        with gr.Row():
                            with gr.Column():
                                ...
                                with gr.TabItem("Text2Video") as video_template_images_tab:
                                    ...
                                    with gr.Row():
                                        sd_model_checkpoint_for_animatediff_text2video = gr.Dropdown(value="majicmixRealistic_v7.safetensors", choices=list(set(
                                            ["Chilloutmix-Ni-pruned-fp16-fix.safetensors"] + checkpoints + external_checkpoints)), elem_id='dropdown', min_width=40, label="The base checkpoint you use for Text2Video(For animatediff only).", visible=True)
                                ...
                                with gr.TabItem("Image2Video") as video_upload_image_tab:
                                    i2v_mode_choose = gr.Radio(value="Base on One Image", elem_id='Radio', choices=[
                                                               "Base on One Image", "From One Image to another"], label="Generate video from one image or more images", show_label=False, visible=True)

                                    with gr.Row():
                                        init_image = gr.Image(
                                            label="Image for easyphoto to Image2Video", show_label=True, elem_id="{id_part}_image", source="upload")
                                        last_image = gr.Image(label="Last image for easyphoto to Image2Video",
                                                              show_label=True, elem_id="{id_part}_image", source="upload", visible=False)
                                    init_image_prompt = gr.Textbox(label="Prompt For Image2Video", value="", show_label=True,
                                                                   visible=True, placeholder="Please write the corresponding prompts using the template.")

                                    def update_i2v_mode(i2v_mode_choose):
                                        if i2v_mode_choose == "Base on One Image":
                                            return [
                                                gr.update(
                                                    label="Image for easyphoto to Image2Video", value=None, visible=True),
                                                gr.update(
                                            ...
                                            return [
                                                gr.update(
                                                    label="First Image for easyphoto to Image2Video", value=None, visible=True),
                                                gr.update(
                                                    value=None, visible=True)
                                            ]
                                    i2v_mode_choose.change(update_i2v_mode, inputs=i2v_mode_choose, outputs=[
                                                           init_image, last_image])

                                    with gr.Row():
                                        sd_model_checkpoint_for_animatediff_image2video = gr.Dropdown(value="majicmixRealistic_v7.safetensors", choices=list(set(
                                            ["Chilloutmix-Ni-pruned-fp16-fix.safetensors"] + checkpoints + external_checkpoints)), elem_id='dropdown', min_width=40, label="The base checkpoint you use for Image2Video(For animatediff only).", visible=True)
                                ...
                                model_selected_tabs = [
                                    video_template_images_tab, video_upload_image_tab, video_upload_video_tab]
                                for i, tab in enumerate(model_selected_tabs):
                                    tab.select(fn=lambda tabnum=i: tabnum, inputs=[
                                    ], outputs=[video_model_selected_tab])
                                ...
                                with gr.Row():
                                    ...
                                    num_of_faceid = gr.Dropdown(value=str(1), elem_id='dropdown', choices=[
                                                                1, 2, 3, 4, 5], label=f"Num of Faceid", visible=False)
                                ...
                                with gr.Accordion("Advanced Options", open=False):
                                    ...
                                    with gr.Row():
                                        ...
                                        video_interpolation.change(lambda x: video_interpolation_ext.update(
                                            visible=x), inputs=[video_interpolation], outputs=[video_interpolation_ext])
                            ...
                            with gr.Column():
                                ...
                                output_video = gr.Video(
                                    label='Output of Video', visible=False)
                                ...
                                save_as.change(update_save_as_mode, [save_as], [
                                               output_video, output_gif])
                                ...
                                with gr.Row():
                                    def save_video():
                                        origin_path = os.path.join(
                                            easyphoto_video_outpath_samples, "origin")
                                        crop_path = os.path.join(
                                            easyphoto_video_outpath_samples, "crop")
                                        if not os.path.exists(origin_path):
                                            os.makedirs(
                                                origin_path, exist_ok=True)
                                        index = len(
                                            [path for path in os.listdir(origin_path)]) + 1

                                        video_path = []
                                        video_crop_path = []
                                        for sub_index in range(max(index - 3, 0), index):
                                            video_mp4_path = os.path.join(
                                                origin_path, str(sub_index).zfill(8) + '.mp4')
                                            video_gif_path = os.path.join(
                                                origin_path, str(sub_index).zfill(8) + '.gif')
                                            ...
                                            video_mp4_crop_path = os.path.join(
                                                crop_path, str(sub_index).zfill(8) + "_crop" + '.mp4')
                                            video_gif_crop_path = os.path.join(
                                                crop_path, str(sub_index).zfill(8) + "_crop" + '.gif')
                                            for _video_path in [video_mp4_crop_path, video_gif_crop_path]:
                                                if os.path.exists(_video_path):
                                                    video_crop_path.append(
                                                        _video_path)
                                                    continue
                                        return gr.File.update(value=video_path, visible=True), gr.File.update(value=video_crop_path, visible=True)

                                    save = gr.Button(
                                        'List Recent Conversion Results', elem_id=f'save')
                                    download_origin_files = gr.File(None, label='Download Files For Origin Video', file_count="multiple",
                                                                    interactive=False, show_label=True, visible=False, elem_id=f'download_files')
                                    download_crop_files = gr.File(None, label='Download Files For Cropped Video', file_count="multiple",
                                                                  interactive=False, show_label=True, visible=False, elem_id=f'download_files')
                                    save.click(fn=save_video, inputs=None, outputs=[
                                               download_origin_files, download_crop_files], show_progress=False)
                        ...
                        display_button.click(
                            ...
                            outputs=[infer_progress, output_video,
                                     output_gif, output_images]
                        )
                    ...
                    with gr.Row():
                        with gr.Column():
                            ...
                            template_image_tryon = gr.Image(
                                label="Image for skybox", elem_id="{id_part}_tem_image", show_label=False, source="upload", tool='sketch')

                            gr.Markdown('Reference Image')
                            ref_image_selected_tab = gr.State(0)
                            with gr.TabItem("cloth gallery") as cloth_gallery_tab:
                                cloth_gallery_dir = os.path.join(
                                    cloth_id_outpath_samples, 'gallery')
                                os.makedirs(cloth_gallery_dir, exist_ok=True)
                                cloth_ids = glob.glob(os.path.join(
                                    cloth_gallery_dir, '*.jpg')) + glob.glob(os.path.join(cloth_gallery_dir, '*.png'))
                                cloth_gallery_list = [
                                    (i, i.split('/')[-1].split('.')[0]) for i in cloth_ids]
                                ...
                                selected_cloth_template_images = gr.Text(
                                    show_label=False, visible=False, placeholder="Selected")
                                cloth_gallery.select(
                                    select_function, None, selected_cloth_template_images)
                            ...
                            with gr.TabItem("upload") as upload_ref_image_tab:
                                main_image = gr.Image(
                                    label="Main Image", elem_id="{id_part}_ref_image", show_label=False, source="upload", type="filepath")
                            ...
                            model_selected_tabs = [
                                cloth_gallery_tab, upload_ref_image_tab]
                            for i, tab in enumerate(model_selected_tabs):
                                tab.select(fn=lambda tabnum=i: tabnum, inputs=[
                        ...
                        with gr.Column():
                            ...
                            tryon_output_images = gr.Gallery(
                                label='Output',
                                show_label=False
                            ).style(columns=[4], rows=[2], object_fit="contain", height="auto")
                            ...
                            selected_tryon_output_image = gr.Text(
                                show_label=False, visible=False, placeholder="Selected")
                            tryon_output_images.select(
                                select_function, tryon_output_images, selected_tryon_output_image)
                            ...
                            with gr.Row():
                                ...
                                buttons_tryon = {
                                    ...
                                    'inpaint': ToolButton_webui('🎨️', elem_id=f'{tabname}_send_to_inpaint', tooltip="Send image and generation parameters to img2img inpaint tab."),
                                    'extras': ToolButton_webui('📐', elem_id=f'{tabname}_send_to_extras', tooltip="Send image and generation parameters to extras tab."),
                                    'easyphoto': ToolButton_webui('📷', elem_id=f'{tabname}_send_to_easyphoto', tooltip="Send image and generation parameters to EasyPhoto Photo Inference tab.")
                                }

                            parameters_copypaste.add_paste_fields(
                                "tryon", template_image_tryon, None)
                            ...
                            parameters_copypaste.add_paste_fields(
                                "easyphoto", init_image_photo_inference, None)
                            ...
                            buttons_photo_infer['tryon'].click(fn=None,
                                                               _js="switch_to_ep_tryon",
                                                               )

                            def select_single_image_upload():
                                upload_way = 'Single Image Upload'
                                return [upload_way, gr.update(visible=False), gr.update(visible=True), gr.update(visible=False)]
    
                            buttons_tryon['easyphoto'].click(fn=select_single_image_upload,
                                                             _js="switch_to_ep_photoinfer_upload",
                                                             inputs=[],
                                                             outputs=[
                                                                 upload_way, template_gallery, single_image_upload, batch_images_upload]
                                                             )
