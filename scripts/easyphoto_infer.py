...
def easyphoto_infer_forward(
    ...
    # global
    global check_hash, models_zero123, model_anyid, ddim_sampler
    ...
    if infer_way == 'Infer with Anydoor':
        ...
        if reference_mask is None:
            # refine
            _, mask_reference = easyphoto_mask_forward(reference_image, "Reference")
        else:
            mask_reference = reference_mask[:, :, 0]
        ...
        if template_mask is None:
            # refine
            _, mask_template = easyphoto_mask_forward(template_image, "Template")
            # update for return
        ...
        mask_reference = mask_reference/255    
        synthesis = inference_single_image(model_anyid, ddim_sampler, img_ref.copy(), mask_reference.copy(), img_template.copy(), mask_template.copy())

        return_res.append(Image.fromarray(synthesis))

        # for final paste
        # _, box_main = mask_to_box(mask_reference))
        # _, box_template = mask_to_box(mask_template)
        # template_copy = copy.deepcopy(img_template)
        # copy.deepcopy(mask_template)


        # return '',[], template_mask, reference_mask
        # enhance with lora
        ...
        # lora_path = os.path.join(models_path, f"Lora/{user_id}.safetensors")
