...
def resize_image(input_image, resolution, nearest=False, crop264=True):
    ...
    if not nearest:
        img = cv2.resize(
            input_image, (W, H), interpolation=cv2.INTER_LANCZOS4 if k > 1 else cv2.INTER_AREA)
    else:
...
def get_controlnet_unit(
    ...
    if unit == "canny":
        ...
        if is_batch:
            ...
            for _input_image in input_image:
                ...
                color_image = cv2.resize(color_image, (int(
                    now_w//blur_ratio), int(now_h//blur_ratio)), interpolation=cv2.INTER_CUBIC)
            ...
            control_unit['batch_images'] = [
                np.array(_input_image, np.uint8) for _input_image in new_input_image]
        else:
            ...
            control_unit['input_image'] = {
                'image': np.asarray(color_image), 'mask': None}
    ...
    if unit != 'color' and not unit.startswith('sdxl') and not unit.startswith('ipa'):
        if is_batch:
            control_unit['batch_images'] = [
                np.array(_input_image, np.uint8) for _input_image in input_image]
        else:
            control_unit['input_image'] = {
                'image': np.asarray(input_image), 'mask': None}
...
def inpaint(
    ...
    controlnet_units_list = []
    w = int(input_image.width) if type(
        input_image) is not list else int(input_image[0].width)
    h = int(input_image.height) if type(
...
def easyphoto_infer_forward(
    ...
    for user_id in user_ids:
        if user_id != "none":
            ...
            sdxl_lora_type = get_lora_type(os.path.join(
                models_path, f"Lora/{user_id}.safetensors"))
    ...
    if control_net_unit_count < max_control_net_unit_count:
        error_info = (
            ...
            "Multi-ControlNet: ControlNet unit number (requires restart).".format(
                max_control_net_unit_count)
        )
        return error_info, [], []
    ...
    if ip_adapter_control:
        ...
        if valid_user_id_num > 1:
            ep_logger.error(
                "EasyPhoto does not support IP-Adapter Control with multiple user ids currently.")
        ...
        if ip_adapter_control and valid_user_id_num != valid_ipa_image_path_num:
            ep_logger.warning(
                ...
                "corresponding to the user instead.".format(
                    valid_user_id_num, valid_ipa_image_path_num)
            )
        if not display_score:
            display_score = True
            ep_logger.warning(
                "Display score is forced to be true when IP-Adapter Control is enabled.")
    ...
    try:
        ...
        if tabs == 0:
            ...
            template_images = [file_d['name']
                               for file_d in uploaded_template_images]
        elif tabs == 3:
            # load sd and vae
            prompt_generate_sd_model_checkpoint_type = get_checkpoint_type(
            ...
            ep_logger.info(
                "Template images will be generated when you use text2photo")
    ...
    if retinaface_detection is None:
        retinaface_detection = pipeline(
            Tasks.face_detection, 'damo/cv_resnet50_face-detection_retinaface', model_revision='v2.0.2')
    if image_face_fusion is None:
        image_face_fusion = pipeline(
            Tasks.image_face_fusion, model='damo/cv_unet-image-face-fusion_damo', model_revision='v1.3')
    if face_skin is None:
        face_skin = Face_Skin(os.path.join(os.path.abspath(os.path.dirname(
            __file__)).replace("scripts", "models"), "face_skin.pth"))
    if skin_retouching is None:
        try:
            skin_retouching = pipeline(
                'skin-retouching-torch', model='damo/cv_unet_skin_retouching_torch', model_revision='v1.0.2')
        except Exception as e:
            torch.cuda.empty_cache()
            traceback.print_exc()
            ep_logger.error(
                f"Skin Retouching model load error. Error Info: {e}")
    if portrait_enhancement is None or old_super_resolution_method != super_resolution_method:
        try:
            ...
            traceback.print_exc()
            ep_logger.error(
                f"Portrait Enhancement model load error. Error Info: {e}")
    ...
    if display_score and face_recognition is None:
        face_recognition = pipeline(
            "face_recognition", model='bubbliiiing/cv_retinafce_recognition', model_revision='v1.0.3')

    # psgan for transfer makeup
    if makeup_transfer and psgan_inference is None:
        try:
            makeup_transfer_model_path = os.path.join(os.path.abspath(
                os.path.dirname(__file__)).replace("scripts", "models"), "makeup_transfer.pth")
            face_landmarks_model_path = os.path.join(os.path.abspath(
                os.path.dirname(__file__)).replace("scripts", "models"), "face_landmarks.pth")
            ...
            traceback.print_exc()
            ep_logger.error(
                f"MakeUp Transfer model load error. Error Info: {e}")
    ...
    if tabs == 3:
        reload_sd_model_vae(
            prompt_generate_sd_model_checkpoint, prompt_generate_vae)

        if scene_id != 'none':
            # scene lora path
            scene_lora_model_path = os.path.join(
                models_path, "Lora", f"{scene_id}.safetensors")
            if not os.path.exists(scene_lora_model_path):
                return "Please check scene lora is exist or not.", [], []
            is_scene_lora, scene_lora_prompt = get_scene_prompt(
                scene_lora_model_path)
            if not is_scene_lora:
                return "Please use the lora trained by ep.", [], []

            # get lora scene prompt
            last_scene_lora_prompt_high_weight = text_to_image_input_prompt + \
                f", <lora:{scene_id}:0.80>, look at viewer, " + \
                f"{validation_prompt}, <lora:{user_ids[0]}:0.25>, "
            last_scene_lora_prompt_low_weight = text_to_image_input_prompt + \
                f", <lora:{scene_id}:0.40>, look at viewer, " + \
                f"{validation_prompt}, <lora:{user_ids[0]}:0.25>, "

            # text to image with scene lora
            ep_logger.info(
                f"Text to Image with prompt: {last_scene_lora_prompt_high_weight} and lora: {scene_lora_model_path}")
            pose_templates = glob.glob(os.path.join(os.path.abspath(os.path.dirname(__file__)).replace("scripts", "models"), 'pose_templates/*.jpg')) + \
                glob.glob(os.path.join(os.path.abspath(os.path.dirname(__file__)).replace(
                    "scripts", "models"), 'pose_templates/*.png'))
            ...
            ep_logger.info(
                f"Hire Fix with prompt: {last_scene_lora_prompt_low_weight} and lora: {scene_lora_model_path}")
            ...
            ep_logger.info(
                f"Text to Image with prompt: {text_to_image_input_prompt}")
            template_images = txt2img(
    ...
    if not sdxl_pipeline_flag:
        reload_sd_model_vae(sd_model_checkpoint,
    ...
    for index, user_id in enumerate(user_ids):
        if user_id == 'none':
            ...
            # get prompt
            input_prompt = f"{validation_prompt}, <lora:{user_id}:{best_lora_weights}>, " + \
                "<lora:FilmVelvia3:0.65>, " + additional_prompt
            ...
            if sdxl_pipeline_flag:
                input_prompt = f"{validation_prompt}, <lora:{user_id}>, " + \
                    additional_prompt

            # get best image after training
            best_outputs_paths = glob.glob(os.path.join(
                user_id_outpath_samples, user_id, "user_weights", "best_outputs", "*.jpg"))
            # get roop image
            if len(best_outputs_paths) > 0:
                face_id_image_path = best_outputs_paths[0]
            else:
                face_id_image_path = os.path.join(
                    user_id_outpath_samples, user_id, "ref_image.jpg")
            roop_image_path = os.path.join(
                user_id_outpath_samples, user_id, "ref_image.jpg")
            ...
            if ip_adapter_control:
                if ipa_image_paths[index] != "none":
                    ...
                    ipa_image = ImageOps.exif_transpose(
                        ipa_image).convert("RGB")
                else:
                ...
                if len(_ipa_retinaface_boxes) == 0:
                    ep_logger.error(
                        "No face is detected in the uploaded image prompt.")
                ...
                if len(_ipa_retinaface_boxes) > 1:
                    ep_logger.warning(
                        ...
                        "Only the left one will be used.".format(
                            len(_ipa_retinaface_boxes))
                    )
    ...
    for template_idx, template_image in enumerate(template_images):
        ...
        try:
            ...
            template_face_safe_boxes, _, _ = call_face_crop(
                retinaface_detection, template_image, multi_user_safecrop_ratio, "crop")
            ...
            if background_restore:
                ...
                for index in range(len(template_face_safe_boxes)):
                    ...
                    output_mask[retinaface_box[1]:retinaface_box[3],
                                retinaface_box[0]:retinaface_box[2]] = 0
                output_mask = Image.fromarray(np.uint8(cv2.dilate(
                    np.array(output_mask), np.ones((32, 32), np.uint8), iterations=1)))
            else:
                if min(template_detected_facenum, len(user_ids) - last_user_id_none_num) > 1:
                    ...
                    for index in range(len(template_face_safe_boxes)):
                        ...
                        if index in passed_userid_list:
                            ...
                            retinaface_box = template_face_safe_boxes[index]
                            output_mask[retinaface_box[1]:retinaface_box[3],
                                        retinaface_box[0]:retinaface_box[2]] = 255
                    output_mask = Image.fromarray(np.uint8(cv2.dilate(np.array(output_mask), np.ones(
                        (64, 64), np.uint8), iterations=1) - cv2.erode(np.array(output_mask), np.ones((32, 32), np.uint8), iterations=1)))
            ...
            for index in range(min(len(template_face_safe_boxes), len(user_ids) - last_user_id_none_num)):
                ...
                if min(len(template_face_safe_boxes), len(user_ids) - last_user_id_none_num) > 1:
                    ...
                    for sub_index in range(len(template_face_safe_boxes)):
                        if index != sub_index:
                            ...
                            loop_template_image[retinaface_box[1]:retinaface_box[3],
                                                retinaface_box[0]:retinaface_box[2]] = 255
                    loop_template_image = Image.fromarray(
                        np.uint8(loop_template_image))

                # Crop the template image to retain only the portion of the portrait
                if crop_face_preprocess:
                    loop_template_crop_safe_boxes, _, _ = call_face_crop(
                        retinaface_detection, loop_template_image, 3, "crop")
                    loop_template_crop_safe_box = loop_template_crop_safe_boxes[0]
                    input_image = copy.deepcopy(loop_template_image).crop(
                ...
                if sdxl_pipeline_flag:
                    ...
                    ratio = math.sqrt(
                        target_area / (input_image.width * input_image.height))
                    new_size = (int(input_image.width * ratio),
                                int(input_image.height * ratio))
                    ep_logger.info("Start resize image from {} to {}.".format(
                        input_image.size, new_size))
                else:
                    input_short_size = 512.0
                    ep_logger.info(
                        "Start Image resize to {}.".format(input_short_size))
                    short_side = min(input_image.width, input_image.height)
                    resize = float(short_side / input_short_size)
                    new_size = (int(input_image.width//resize),
                                int(input_image.height//resize))
                ...
                replaced_input_image = crop_and_paste(face_id_images[index], face_id_retinaface_masks[index], input_image,
                                                      face_id_retinaface_keypoints[index], input_image_retinaface_keypoint, face_id_retinaface_boxes[index])
                replaced_input_image = Image.fromarray(
                    np.uint8(replaced_input_image))
                ...
                if ip_adapter_control:
                    ...
                    ipa_face_width = ipa_retinaface_box[2] - \
                        ipa_retinaface_box[0]

                    if not ipa_face_part_only:
                        ipa_mask = face_skin(ipa_images[index], retinaface_detection, needs_index=[
                                             [1, 2, 3, 4, 5, 10, 11, 12, 13]])[0]
                        ipa_kernel_size = np.ones(
                            (int(ipa_face_width//10), int(ipa_face_width//10)), np.uint8)
                        # Fill small holes with a close operation (w/o cv2.dilate)
                        ipa_mask = Image.fromarray(np.uint8(cv2.morphologyEx(
                            np.array(ipa_mask), cv2.MORPH_CLOSE, ipa_kernel_size)))
                    else:
                        # Expand the reference image in the x-axis direction to include the ears.
                        h, w, c = np.shape(ipa_retinaface_mask)
                        ipa_mask = np.zeros_like(
                            np.array(ipa_retinaface_mask, np.uint8))
                        ipa_retinaface_box[0] = np.clip(
                            np.array(ipa_retinaface_box[0], np.int32) - ipa_face_width * 0.15, 0, w - 1)
                        ipa_retinaface_box[2] = np.clip(
                            np.array(ipa_retinaface_box[2], np.int32) + ipa_face_width * 0.15, 0, w - 1)
                        ipa_mask[ipa_retinaface_box[1]:ipa_retinaface_box[3],
                                 ipa_retinaface_box[0]:ipa_retinaface_box[2]] = 255
                    ...
                    ipa_image_face = ImageChops.multiply(
                        ipa_images[index], ipa_mask)
                    ...
                    ipa_image_face = ImageOps.pad(
                        ipa_image_face, padded_size, color=(255, 255, 255))
                ...
                if roop_images[index] is not None and apply_face_fusion_before:
                    fusion_image = image_face_fusion(
                        dict(template=input_image, user=roop_images[index]))[OutputKeys.OUTPUT_IMG]
                    fusion_image = Image.fromarray(
                        cv2.cvtColor(fusion_image, cv2.COLOR_BGR2RGB))

                    # The edge shadows generated by fusion are filtered out by taking intersections of masks of faces before and after fusion.
                    # detect face area
                    fusion_image_mask = np.int32(np.float32(face_skin(
                        fusion_image, retinaface_detection, needs_index=[[1, 2, 3, 10, 11, 12, 13]])[0]) > 128)
                    input_image_mask = np.int32(np.float32(face_skin(
                        input_image, retinaface_detection, needs_index=[[1, 2, 3, 10, 11, 12, 13]])[0]) > 128)
                    combine_mask = cv2.blur(
                        np.uint8(input_image_mask * fusion_image_mask * 255), (8, 8)) / 255

                    # paste back to photo
                    fusion_image = np.array(
                        fusion_image) * combine_mask + np.array(input_image) * (1 - combine_mask)
                    fusion_image = Image.fromarray(np.uint8(fusion_image))

                    input_image = Image.fromarray(np.uint8((np.array(input_image, np.float32) * (
                        1 - before_face_fusion_ratio) + np.array(fusion_image, np.float32) * before_face_fusion_ratio)))

                if input_mask_face_part_only:
                    face_width = input_image_retinaface_box[2] - \
                        input_image_retinaface_box[0]
                    input_mask = face_skin(input_image, retinaface_detection, needs_index=[
                                           [1, 2, 3, 4, 5, 10, 11, 12, 13]])[0]

                    kernel_size = np.ones(
                        (int(face_width//10), int(face_width//10)), np.uint8)
                    # Fill small holes with a close operation
                    input_mask = Image.fromarray(np.uint8(cv2.morphologyEx(
                        np.array(input_mask), cv2.MORPH_CLOSE, kernel_size)))
                    # Use dilate to reconstruct the surrounding area of the face
                    input_mask = Image.fromarray(
                        np.uint8(cv2.dilate(np.array(input_mask), kernel_size, iterations=1)))
                else:
                    ...
                    input_image_retinaface_box = np.int32(
                        input_image_retinaface_box)

                    face_width = input_image_retinaface_box[2] - \
                        input_image_retinaface_box[0]
                    input_image_retinaface_box[0] = np.clip(np.array(
                        input_image_retinaface_box[0], np.int32) - face_width * 0.10, 0, w - 1)
                    input_image_retinaface_box[2] = np.clip(np.array(
                        input_image_retinaface_box[2], np.int32) + face_width * 0.10, 0, w - 1)
                ...
                template_image_original_face_area = np.array(original_input_template)[
                    input_image_retinaface_box[1]:input_image_retinaface_box[3], input_image_retinaface_box[0]:input_image_retinaface_box[2], :]

                # First diffusion, facial reconstruction
                ep_logger.info("Start First diffusion.")
                first_inpaint_area = ImageChops.multiply(
                    input_image, input_mask)
                if not face_shape_match:
                    if not sdxl_pipeline_flag:
                        controlnet_pairs = [["canny", input_image, 0.50], [
                            "openpose", replaced_input_image, 0.50], ["color", input_image, 0.85]]
                        if ip_adapter_control:
                            controlnet_pairs.append(
                                ["ipa_full_face", ipa_image_face, ip_adapter_weight])
                    else:
                        controlnet_pairs = [
                            ["sdxl_canny_mid", input_image, 0.50]]
                        if ip_adapter_control:
                            controlnet_pairs.append(
                                ["ipa_sdxl_plus_face", ipa_image_face, ip_adapter_weight])
                    first_diffusion_output_image = inpaint(input_image, input_mask, controlnet_pairs, diffusion_steps=first_diffusion_steps,
                                                           denoising_strength=first_denoising_strength, input_prompt=input_prompts[index], hr_scale=1.0, seed=str(seed))
                else:
                    if not sdxl_pipeline_flag:
                        controlnet_pairs = [["canny", input_image, 0.50], [
                        ...
                        if ip_adapter_control:
                            controlnet_pairs.append(
                                ["ipa_sdxl_plus_face", ipa_image_face, ip_adapter_weight])
                    first_diffusion_output_image = inpaint(input_image, None, controlnet_pairs, diffusion_steps=first_diffusion_steps,
                                                           denoising_strength=first_denoising_strength, input_prompt=input_prompts[index], hr_scale=1.0, seed=str(seed))

                    # detect face area
                    face_skin_mask = face_skin(first_diffusion_output_image, retinaface_detection, needs_index=[
                                               [1, 2, 3, 4, 5, 7, 8, 10, 11, 12, 13]])[0]
                    kernel_size = np.ones(
                        (int(face_width//10), int(face_width//10)), np.uint8)

                    # Fill small holes with a close operation
                    face_skin_mask = Image.fromarray(np.uint8(cv2.morphologyEx(
                        np.array(face_skin_mask), cv2.MORPH_CLOSE, kernel_size)))

                    # Use dilate to reconstruct the surrounding area of the face
                    face_skin_mask = Image.fromarray(
                        np.uint8(cv2.dilate(np.array(face_skin_mask), kernel_size, iterations=1)))
                    face_skin_mask = cv2.blur(
                        np.float32(face_skin_mask), (32, 32)) / 255
                    ...
                    input_image_uint8 = np.array(
                        first_diffusion_output_image) * face_skin_mask + np.array(input_image) * (1 - face_skin_mask)
                    first_diffusion_output_image = Image.fromarray(
                        np.uint8(input_image_uint8))

                if color_shift_middle:
                    # apply color shift
                    ep_logger.info("Start color shift middle.")
                    first_diffusion_output_image_uint8 = np.uint8(
                        np.array(first_diffusion_output_image))
                    # crop image first
                    ...
                    # apply color shift
                    first_diffusion_output_image_crop_color_shift = np.array(
                        copy.deepcopy(first_diffusion_output_image_crop))
                    ...
                    # detect face area
                    face_skin_mask = np.float32(face_skin(first_diffusion_output_image_crop, retinaface_detection, needs_index=[
                                                [1, 2, 3, 4, 5, 10, 11, 12, 13]])[0])
                    face_skin_mask = cv2.blur(face_skin_mask, (32, 32)) / 255

                    # paste back to photo
                    first_diffusion_output_image_uint8[input_image_retinaface_box[1]:input_image_retinaface_box[3], input_image_retinaface_box[0]:input_image_retinaface_box[2], :] = \
                        first_diffusion_output_image_crop_color_shift * face_skin_mask + \
                        np.array(first_diffusion_output_image_crop) * \
                        (1 - face_skin_mask)
                    first_diffusion_output_image = Image.fromarray(
                        np.uint8(first_diffusion_output_image_uint8))
                ...
                if roop_images[index] is not None and apply_face_fusion_after:
                    ...
                    fusion_image = image_face_fusion(dict(
                        template=first_diffusion_output_image, user=roop_images[index]))[OutputKeys.OUTPUT_IMG]
                    fusion_image = Image.fromarray(
                        cv2.cvtColor(fusion_image, cv2.COLOR_BGR2RGB))
                    ...
                    fusion_image_mask = np.int32(np.float32(face_skin(
                        fusion_image, retinaface_detection, needs_index=[[1, 2, 3, 11, 12, 13]])[0]) > 128)
                    input_image_mask = np.int32(np.float32(face_skin(
                        first_diffusion_output_image, retinaface_detection, needs_index=[[1, 2, 3, 11, 12, 13]])[0]) > 128)
                    combine_mask = cv2.blur(
                        np.uint8(input_image_mask * fusion_image_mask * 255), (8, 8)) / 255

                    # paste back to photo
                    fusion_image = np.array(fusion_image) * combine_mask + np.array(
                        first_diffusion_output_image) * (1 - combine_mask)
                    fusion_image = Image.fromarray(np.uint8(fusion_image))

                    input_image = Image.fromarray(np.uint8((np.array(first_diffusion_output_image, np.float32) * (
                        1 - after_face_fusion_ratio) + np.array(fusion_image, np.float32) * after_face_fusion_ratio)))
                else:
                ...
                if need_mouth_fix:
                    ...
                    mouth_mask, face_mask = face_skin(input_image, retinaface_detection, [
                                                      [4, 5, 12, 13], [1, 2, 3, 4, 5, 10, 11, 12, 13]])
                    # Obtain the mask of the area around the face
                    face_mask = Image.fromarray(np.uint8(cv2.dilate(np.array(face_mask), np.ones(
                        (32, 32), np.uint8), iterations=1) - cv2.erode(np.array(face_mask), np.ones((16, 16), np.uint8), iterations=1)))
                    ...
                    input_mask = Image.fromarray(
                        np.uint8(np.clip(np.float32(face_mask) + np.float32(mouth_mask), 0, 255)))

                ep_logger.info("Start Second diffusion.")
                if not sdxl_pipeline_flag:
                    controlnet_pairs = [["canny", fusion_image, 1.00], [
                        "tile", fusion_image, 1.00]]
                    if ip_adapter_control:
                        controlnet_pairs = [["canny", fusion_image, 1.00], [
                            "ipa_full_face", ipa_image_face, ip_adapter_weight]]
                else:
                ...
                second_diffusion_output_image = inpaint(
                    input_image, input_mask, controlnet_pairs, input_prompts[index], diffusion_steps=second_diffusion_steps, denoising_strength=second_denoising_strength, hr_scale=default_hr_scale, seed=str(seed))

                # use original template face area to shift generated face color at last
                if color_shift_last:
                    ep_logger.info("Start color shift last.")
                    # scale box
                    rescale_retinaface_box = [
                        int(i * default_hr_scale) for i in input_image_retinaface_box]
                    second_diffusion_output_image_uint8 = np.uint8(
                        np.array(second_diffusion_output_image))
                    ...
                    # apply color shift
                    second_diffusion_output_image_crop_color_shift = np.array(
                        copy.deepcopy(second_diffusion_output_image_crop))
                    ...
                    # detect face area
                    face_skin_mask = np.float32(face_skin(
                        second_diffusion_output_image_crop, retinaface_detection, needs_index=[[1, 2, 3, 4, 5, 10]])[0])
                    face_skin_mask = cv2.blur(face_skin_mask, (32, 32)) / 255

                    # paste back to photo
                    second_diffusion_output_image_uint8[rescale_retinaface_box[1]:rescale_retinaface_box[3], rescale_retinaface_box[0]:rescale_retinaface_box[2], :] = \
                        second_diffusion_output_image_crop_color_shift * face_skin_mask + \
                        np.array(second_diffusion_output_image_crop) * \
                        (1 - face_skin_mask)
                    second_diffusion_output_image = Image.fromarray(
                ...
                if makeup_transfer:
                    ...
                    second_diffusion_output_image_crop = Image.fromarray(
                        second_diffusion_output_image_uint8[rescale_retinaface_box[1]:rescale_retinaface_box[3], rescale_retinaface_box[0]:rescale_retinaface_box[2], :])
                    template_image_original_face_area = Image.fromarray(
                        np.uint8(template_image_original_face_area))

                    # makeup transfer
                    second_diffusion_output_image_crop_makeup_transfer = second_diffusion_output_image_crop.resize([
                                                                                                                   256, 256])
                    template_image_original_face_area = Image.fromarray(
                        np.uint8(template_image_original_face_area)).resize([256, 256])
                    ...
                    # detect face area
                    face_skin_mask = np.float32(face_skin(second_diffusion_output_image_crop, retinaface_detection, needs_index=[
                                                [1, 2, 3, 4, 5, 10, 11, 12, 13]])[0])
                    face_skin_mask = cv2.blur(
                        face_skin_mask, (32, 32)) / 255 * makeup_transfer_ratio

                    # paste back to photo
                    second_diffusion_output_image_uint8[rescale_retinaface_box[1]:rescale_retinaface_box[3], rescale_retinaface_box[0]:rescale_retinaface_box[2], :] = \
                        np.array(second_diffusion_output_image_crop_makeup_transfer) * face_skin_mask + \
                        np.array(second_diffusion_output_image_crop) * \
                        (1 - face_skin_mask)
                    second_diffusion_output_image = Image.fromarray(
                        np.uint8(np.clip(second_diffusion_output_image_uint8, 0, 255)))

                # If it is a large template for cutting, paste the reconstructed image back
                if crop_face_preprocess:
                    ep_logger.info(
                        "Start paste crop image to origin template.")
                    origin_loop_template_image = np.array(
                        copy.deepcopy(loop_template_image))

                    x1, y1, x2, y2 = loop_template_crop_safe_box
                    second_diffusion_output_image = second_diffusion_output_image.resize(
                        [x2-x1, y2-y1], Image.Resampling.LANCZOS)
                    origin_loop_template_image[y1:y2, x1:x2] = np.array(
                        second_diffusion_output_image)

                    loop_output_image = Image.fromarray(
                        np.uint8(origin_loop_template_image))
                else:
                    loop_output_image = second_diffusion_output_image.resize(
                        [loop_template_image.width, loop_template_image.height])
                ...
                if display_score:
                    ...
                    embedding = face_recognition(dict(user=Image.fromarray(
                        np.uint8(loop_output_image_face))))[OutputKeys.IMG_EMBEDDING]
                    roop_image_embedding = face_recognition(dict(user=Image.fromarray(
                        np.uint8(roop_images[index]))))[OutputKeys.IMG_EMBEDDING]
                    loop_output_image_faceid = np.dot(
                        embedding, np.transpose(roop_image_embedding))[0][0]
                    face_id_outputs.append((roop_images[index], "{:.2f}, {}, the reference image".format(
                        loop_output_image_faceid, user_ids[index])))
                    if ip_adapter_control:
                        ipa_image_embedding = face_recognition(dict(user=Image.fromarray(
                            np.uint8(ipa_images[index]))))[OutputKeys.IMG_EMBEDDING]
                        ipa_image_faceid = np.dot(
                            embedding, np.transpose(ipa_image_embedding))[0][0]
                        face_id_outputs.append((ipa_images[index], "{:.2f}, {}, the image prompt".format(
                            ipa_image_faceid, user_ids[index])))
                    loop_output_image = Image.fromarray(loop_output_image)

                if min(len(template_face_safe_boxes), len(user_ids) - last_user_id_none_num) > 1:
                    ep_logger.info(
                        "Start paste crop image to origin template in multi people.")
                    template_face_safe_box = template_face_safe_boxes[index]
                    output_image_mask = np.zeros_like(np.array(output_image))
                    output_image_mask[template_face_safe_box[1]:template_face_safe_box[3],
                                      template_face_safe_box[0]:template_face_safe_box[2]] = 255
                    output_image_mask = cv2.blur(
                        output_image_mask, (32, 32)) / 255

                    output_image = np.array(loop_output_image, np.float32) * \
                        output_image_mask + \
                        np.array(output_image) * (1 - output_image_mask)
                ...
                if min(len(template_face_safe_boxes), len(user_ids) - last_user_id_none_num) > 1 or background_restore:
                    ...
                    if not background_restore:
                        ...
                        left, top, right, bottom = [
                            np.min(np.array(template_face_safe_boxes)[:, 0]) - background_padding_size, np.min(
                                np.array(template_face_safe_boxes)[:, 1]) - background_padding_size,
                            np.max(np.array(template_face_safe_boxes)[:, 2]) + background_padding_size, np.max(
                                np.array(template_face_safe_boxes)[:, 3]) + background_padding_size
                        ]
                        ...
                        left, top = int(np.clip(center_x - long_side // 2, 0, w - 1)
                                        ), int(np.clip(center_y - long_side // 2, 0, h - 1))
                        right, bottom = int(
                            np.clip(left + long_side, 0, w - 1)), int(np.clip(top + long_side, 0, h - 1))

                        # Crop image and mask for Diffusion
                        sub_output_image = output_image.crop(
                            [left, top, right, bottom])
                        sub_output_mask = output_mask.crop(
                            [left, top, right, bottom])
                        ...
                        short_side = min(sub_output_image.width,
                                         sub_output_image.height)
                        if min(sub_output_image.width, sub_output_image.height) > target_short_side:
                            resize = float(short_side / target_short_side)
                        else:
                            resize = 1
                        new_size = (int(sub_output_image.width // resize // 32 * 32),
                                    int(sub_output_image.height // resize // 32 * 32))
                        sub_output_image = sub_output_image.resize(
                            new_size, Image.Resampling.LANCZOS)

                        # Diffusion
                        if not sdxl_pipeline_flag:
                            controlnet_pairs = [["canny", sub_output_image, 1.00, 1], [
                                "color", sub_output_image, 1.00, 1]]
                        else:
                            controlnet_pairs = [
                                ["sdxl_canny_mid", sub_output_image, 1.00, 1]]
                        sub_output_image = inpaint(sub_output_image, sub_output_mask, controlnet_pairs,
                                                   input_prompt_without_lora, 30, denoising_strength=denoising_strength, hr_scale=1, seed=str(seed))

                        # Paste the image back to the background
                        sub_output_image = sub_output_image.resize(
                            [long_side, long_side])
                        output_image = np.array(output_image)
                        output_image[top:bottom, left:right] = np.array(
                            sub_output_image)
                        output_image = Image.fromarray(output_image)
                    else:
                        short_side = min(output_image.width,
                                         output_image.height)
                        # get target_short_side base on the ratio of width and height
                        if output_image.width / output_image.height > 1.5 or output_image.height / output_image.width > 1.5:
                            ...
                            target_short_side = 768
                        resize = float(short_side / target_short_side)
                        new_size = (int(output_image.width//resize),
                                    int(output_image.height//resize))
                        output_image = output_image.resize(
                            new_size, Image.Resampling.LANCZOS)

                        if not sdxl_pipeline_flag:
                            controlnet_pairs = [["canny", output_image, 1.00, 1], [
                                "color", output_image, 1.00, 1]]
                        else:
                            controlnet_pairs = [
                                ["sdxl_canny_mid", output_image, 1.00, 1]]
                        output_image = inpaint(output_image, output_mask, controlnet_pairs, input_prompt_without_lora,
                                               30, denoising_strength=denoising_strength, hr_scale=1, seed=str(seed))

            except Exception as e:
                torch.cuda.empty_cache()
                traceback.print_exc()
                ep_logger.error(
                    f"Background Restore Failed, Please check the ratio of height and width in template. Error Info: {e}")
            ...
            if total_processed_person != 0:
                if skin_retouching_bool:
                    try:
                        ...
                        output_image = Image.fromarray(cv2.cvtColor(skin_retouching(output_image)[
                                                       OutputKeys.OUTPUT_IMG], cv2.COLOR_BGR2RGB))
                    except Exception as e:
                        torch.cuda.empty_cache()
                        traceback.print_exc()
                ...
                if super_resolution:
                    try:
                        ...
                        output_image = Image.fromarray(cv2.cvtColor(portrait_enhancement(
                            output_image)[OutputKeys.OUTPUT_IMG], cv2.COLOR_BGR2RGB))
                    except Exception as e:
                        torch.cuda.empty_cache()
                        traceback.print_exc()
            ...
            save_image(output_image, easyphoto_outpath_samples, "EasyPhoto", None, None, opts.grid_format,
                       info=None, short_filename=not opts.grid_extended_filename, grid=True, p=None)
            ...
            traceback.print_exc()
            ep_logger.error(
                f"Template {str(template_idx + 1)} error: Error info is {e}, skip it.")
...
def easyphoto_video_infer_forward(
    ...
    checkpoint_type_image2video = get_checkpoint_type(
    ...
    try:
        ...
        if tabs == 0:  # t2v
            ...
            template_images, actual_fps = get_mov_all_images(
                init_video, max_fps)
            template_images = [template_images[:max_frames]
                               ] if max_frames != -1 else [template_images]
    except Exception as e:
        torch.cuda.empty_cache()
        traceback.print_exc()
    ...
    if tabs == 0:
        reload_sd_model_vae(
            sd_model_checkpoint_for_animatediff_text2video, "vae-ft-mse-840000-ema-pruned.ckpt")
        ...
        reload_sd_model_vae(
            sd_model_checkpoint_for_animatediff_image2video, "vae-ft-mse-840000-ema-pruned.ckpt")
    ...
    for template_idx, template_image in enumerate(template_images):
        ...
        try:
            # open the template image
            template_image = [Image.fromarray(np.uint8(_)).convert(
                "RGB") for _ in template_image]
            loop_template_image = copy.deepcopy(template_image)
            ...
            for idx, _input_image in enumerate(input_image):
                ...
                resize = float(short_side / 512.0)
                new_size = (int(_input_image.width//resize),
                            int(_input_image.height//resize))
            ...
            for idx, _input_image in enumerate(input_image):
                ...
                if len(_input_image_retinaface_boxes) == 0:
                    ...
                    input_image_retinaface_boxes.append(
                        _input_image_retinaface_box)
                    input_image_retinaface_keypoints.append(
            ...
            for _input_image, _input_image_retinaface_box, _input_image_retinaface_keypoint, _input_mask in zip(input_image, input_image_retinaface_boxes, input_image_retinaface_keypoints, input_masks):
                ...
                # Paste user images onto template images
                _replaced_input_image = crop_and_paste(face_id_images[0], face_id_retinaface_masks[0], _input_image,
                                                       face_id_retinaface_keypoints[0], _input_image_retinaface_keypoint, face_id_retinaface_boxes[0])
                _replaced_input_image = Image.fromarray(
                    np.uint8(_replaced_input_image))
                replaced_input_image.append(_replaced_input_image)

                # Fusion of user reference images and input images as canny input
                if roop_images[0] is not None and apply_face_fusion_before:
                    _fusion_image = image_face_fusion(
                        dict(template=_input_image, user=roop_images[0]))[OutputKeys.OUTPUT_IMG]
                    _fusion_image = Image.fromarray(
                        cv2.cvtColor(_fusion_image, cv2.COLOR_BGR2RGB))

                    # The edge shadows generated by fusion are filtered out by taking intersections of masks of faces before and after fusion.
                    # detect face area
                    _fusion_image_mask = np.int32(np.float32(face_skin(
                        _fusion_image, retinaface_detection, needs_index=[[1, 2, 3, 4, 5, 10, 11, 12, 13]])[0]) > 128)
                    _input_image_mask = np.int32(np.float32(face_skin(
                        _input_image, retinaface_detection, needs_index=[[1, 2, 3, 4, 5, 10, 11, 12, 13]])[0]) > 128)
                    # paste back to photo
                    _fusion_image = _fusion_image * _fusion_image_mask * _input_image_mask + \
                        np.array(_input_image) * \
                    ...
                    _input_image = Image.fromarray(np.uint8((np.array(_input_image, np.float32) * (
                        1 - before_face_fusion_ratio) + np.array(_fusion_image, np.float32) * before_face_fusion_ratio)))

                if input_mask_face_part_only:
                    face_width = _input_image_retinaface_box[2] - \
                        _input_image_retinaface_box[0]
                    _input_mask = face_skin(_input_image, retinaface_detection, needs_index=[
                                            [1, 2, 3, 4, 5, 10, 11, 12, 13]])[0]

                    kernel_size = np.ones(
                        (int(face_width//10), int(face_width//10)), np.uint8)
                    # Fill small holes with a close operation
                    _input_mask = Image.fromarray(np.uint8(cv2.morphologyEx(
                        np.array(_input_mask), cv2.MORPH_CLOSE, kernel_size)))
                    # Use dilate to reconstruct the surrounding area of the face
                    _input_mask = Image.fromarray(
                        np.uint8(cv2.dilate(np.array(_input_mask), kernel_size, iterations=1)))
                else:
                    # Expand the template image in the x-axis direction to include the ears.
                    h, w, c = np.shape(_input_mask)
                    _input_mask = np.zeros_like(
                        np.array(_input_mask, np.uint8))
                    _input_image_retinaface_box = np.int32(
                        _input_image_retinaface_box)

                    face_width = _input_image_retinaface_box[2] - \
                        _input_image_retinaface_box[0]
                    _input_image_retinaface_box[0] = np.clip(np.array(
                        _input_image_retinaface_box[0], np.int32) - face_width * 0.10, 0, w - 1)
                    _input_image_retinaface_box[2] = np.clip(np.array(
                        _input_image_retinaface_box[2], np.int32) + face_width * 0.10, 0, w - 1)

                    # get new input_mask
                    _input_mask[_input_image_retinaface_box[1]:_input_image_retinaface_box[3],
                                _input_image_retinaface_box[0]:_input_image_retinaface_box[2]] = 255
                    _input_mask = Image.fromarray(np.uint8(_input_mask))

                # here we get the retinaface_box, we should use this Input box and face pixel to refine the output face pixel colors
                _template_image_original_face_area = np.array(original_input_template)[
                    _input_image_retinaface_box[1]:_input_image_retinaface_box[3], _input_image_retinaface_box[0]:_input_image_retinaface_box[2], :]

                new_input_image.append(_input_image)
                new_input_mask.append(_input_mask)
                template_image_original_face_area.append(
            ...
            ep_logger.info("Start First diffusion.")
            if not face_shape_match:
                controlnet_pairs = [["canny", input_image, 0.50, 1], [
                    "openpose", replaced_input_image, 0.50, 1], ["color", input_image, 0.85, 1]]
            else:
                controlnet_pairs = [["canny", input_image, 0.50, 1], [
            ...
            sum_input_mask = Image.fromarray(
                np.uint8(np.max(np.array(sum_input_mask), axis=0)))
            ...
            first_diffusion_output_image = inpaint(input_image, sum_input_mask, controlnet_pairs, diffusion_steps=first_diffusion_steps, denoising_strength=first_denoising_strength, input_prompt=input_prompts[0], hr_scale=1.0, seed=str(
                seed), sd_model_checkpoint=sd_model_checkpoint, default_positive_prompt=DEFAULT_POSITIVE_AD, default_negative_prompt=DEFAULT_NEGATIVE_AD, animatediff_flag=True, animatediff_fps=int(actual_fps))
            ...
            for idx, [_first_diffusion_output_image, _loop_template_image, _loop_template_crop_safe_box, _input_image_retinaface_box, _template_image_original_face_area] in enumerate(zip(first_diffusion_output_image, loop_template_image, loop_template_crop_safe_box, input_image_retinaface_boxes, template_image_original_face_area)):
                if _input_image_retinaface_box is not None:
                    ...
                    if color_shift_middle:
                        ...
                        _first_diffusion_output_image_uint8 = np.uint8(
                            np.array(_first_diffusion_output_image))
                        # crop image first
                        ...
                        # apply color shift
                        _first_diffusion_output_image_crop_color_shift = np.array(
                            copy.deepcopy(_first_diffusion_output_image_crop))
                        ...
                        # detect face area
                        face_skin_mask = np.float32(face_skin(_first_diffusion_output_image_crop, retinaface_detection, needs_index=[
                                                    [1, 2, 3, 4, 5, 10, 11, 12, 13]])[0])
                        face_skin_mask = cv2.blur(
                            face_skin_mask, (32, 32)) / 255

                        # paste back to photo
                        _first_diffusion_output_image_uint8[_input_image_retinaface_box[1]:_input_image_retinaface_box[3], _input_image_retinaface_box[0]:_input_image_retinaface_box[2], :] = \
                            _first_diffusion_output_image_crop_color_shift * face_skin_mask + \
                            np.array(_first_diffusion_output_image_crop) * \
                            (1 - face_skin_mask)
                        _first_diffusion_output_image = Image.fromarray(
                            np.uint8(_first_diffusion_output_image_uint8))

                    if roop_images[0] is not None and apply_face_fusion_after:
                        # Fusion of facial photos with user photos
                        ep_logger.info(f"Start {idx} second face fusion.")
                        # swap_face(target_img=output_image, source_img=roop_image, model="inswapper_128.onnx", upscale_options=UpscaleOptions())
                        _fusion_image = image_face_fusion(dict(
                            template=_first_diffusion_output_image, user=roop_images[0]))[OutputKeys.OUTPUT_IMG]
                        _fusion_image = Image.fromarray(
                            cv2.cvtColor(_fusion_image, cv2.COLOR_BGR2RGB))
                        ...
                        _fusion_image_mask = np.int32(np.float32(face_skin(
                            _fusion_image, retinaface_detection, needs_index=[[1, 2, 3, 4, 5, 10, 11, 12, 13]])[0]) > 128)
                        _input_image_mask = np.int32(np.float32(face_skin(
                            _first_diffusion_output_image, retinaface_detection, needs_index=[[1, 2, 3, 4, 5, 10, 11, 12, 13]])[0]) > 128)
                        # paste back to photo
                        _fusion_image = _fusion_image * _fusion_image_mask * _input_image_mask + \
                            np.array(_first_diffusion_output_image) * \
                            (1 - _fusion_image_mask * _input_image_mask)
                        _fusion_image = cv2.medianBlur(
                            np.uint8(_fusion_image), 3)
                        _fusion_image = Image.fromarray(_fusion_image)

                        _input_image = Image.fromarray(np.uint8((np.array(_first_diffusion_output_image, np.float32) * (
                            1 - after_face_fusion_ratio) + np.array(_fusion_image, np.float32) * after_face_fusion_ratio)))
                    else:
                    ...
                    if makeup_transfer:
                        ...
                        _template_image_original_face_area = Image.fromarray(
                            np.uint8(_template_image_original_face_area))

                        # makeup transfer
                        _input_image_crop_makeup_transfer = _input_image_crop.resize([
                                                                                     256, 256])
                        _template_image_original_face_area = Image.fromarray(
                            np.uint8(_template_image_original_face_area)).resize([256, 256])
                        ...
                        # detect face area
                        face_skin_mask = np.float32(face_skin(_input_image_crop, retinaface_detection, needs_index=[
                                                    [1, 2, 3, 4, 5, 10, 11, 12, 13]])[0])
                        face_skin_mask = cv2.blur(
                            face_skin_mask, (32, 32)) / 255 * makeup_transfer_ratio
                        ...
                        # paste back to photo
                        _input_image_uint8[_input_image_retinaface_box[1]:_input_image_retinaface_box[3], _input_image_retinaface_box[0]:_input_image_retinaface_box[2], :] = \
                            np.array(_input_image_crop_makeup_transfer) * face_skin_mask + \
                            np.array(_input_image_crop) * (1 - face_skin_mask)
                        _input_image = Image.fromarray(
                            np.uint8(np.clip(_input_image_uint8, 0, 255)))
                else:
                ...
                if crop_face_preprocess:
                    if _loop_template_crop_safe_box is not None:
                        ep_logger.info(
                            f"Start {idx} paste crop image to origin template.")

                        x1, y1, x2, y2 = _loop_template_crop_safe_box
                        _loop_template_image = np.array(_loop_template_image)
                        _loop_template_image[y1:y2, x1:x2] = np.array(
                            _input_image.resize([x2-x1, y2-y1], Image.Resampling.LANCZOS))
                    ...
                    _input_image = Image.fromarray(
                        np.uint8(_loop_template_image))

                if skin_retouching_bool:
                    try:
                        ep_logger.info(f"Start {idx} Skin Retouching.")
                        # Skin Retouching is performed here.
                        _input_image = Image.fromarray(cv2.cvtColor(skin_retouching(_input_image)[
                                                       OutputKeys.OUTPUT_IMG], cv2.COLOR_BGR2RGB))
                    except Exception as e:
                        torch.cuda.empty_cache()
                        traceback.print_exc()
                ...
                if super_resolution:
                    try:
                        ...
                        # Super-resolution is performed here.
                        _input_image = Image.fromarray(cv2.cvtColor(portrait_enhancement(
                            _input_image)[OutputKeys.OUTPUT_IMG], cv2.COLOR_BGR2RGB))
                    except Exception as e:
                        torch.cuda.empty_cache()
                        traceback.print_exc()
                        ep_logger.error(f"Portrait enhancement error: {e}")
                ...
                if display_score:
                    try:
                        # count face id
                        embedding = face_recognition(dict(user=Image.fromarray(np.uint8(_input_image))))[
                            OutputKeys.IMG_EMBEDDING]
                        roop_image_embedding = face_recognition(
                            dict(user=Image.fromarray(np.uint8(roop_images[0]))))[OutputKeys.IMG_EMBEDDING]
                        loop_output_image_faceid = np.dot(
                            embedding, np.transpose(roop_image_embedding))[0][0]

                        # define font and label
                        _input_image = cv2.putText(np.array(_input_image, np.uint8), 'frame_idx: {}, similarity score: {:.2f}'.format(
                            frame_idx, loop_output_image_faceid), (40, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
            ...
            if video_interpolation:
                modelscope_models_to_cpu()
                _outputs = [np.array(_output, np.uint8)
                            for _output in _outputs]
                _outputs, actual_fps = FIRE_forward(
                    _outputs, actual_fps, os.path.join(os.path.abspath(
                        os.path.dirname(__file__)).replace("scripts", "models"), "flownet.pkl"),
                    video_interpolation_ext, 1, fp16=False
                )
                _outputs = [Image.fromarray(np.uint8(_output))
                            for _output in _outputs]
                modelscope_models_to_gpu()

            output_video, output_gif, prefix = convert_to_video(os.path.join(
                easyphoto_video_outpath_samples, "origin"), _outputs, actual_fps, mode=save_as)

            if crop_at_last:
                # get max box of face
                last_retinaface_box = []
                for _output in _outputs:
                    _last_retinaface_boxes, _, _ = call_face_crop(
                        retinaface_detection, _output, crop_at_last_ratio, "last_image")
                ...
                last_retinaface_box = [np.min(last_retinaface_box[:, 0]), np.min(
                    last_retinaface_box[:, 1]), np.max(last_retinaface_box[:, 2]), np.max(last_retinaface_box[:, 3])]

                # make width and height can be divisible by 2
                width, height = (last_retinaface_box[2] - last_retinaface_box[0]) // 2 * \
                    2, (last_retinaface_box[3] -
                        last_retinaface_box[1]) // 2 * 2
                last_retinaface_box = [last_retinaface_box[0], last_retinaface_box[1],
                                       last_retinaface_box[0] + width, last_retinaface_box[1] + height]

                # crop
                _new_outputs = []
                for _output in _outputs:
                    _new_outputs.append(_output.crop(last_retinaface_box))
                output_video, output_gif, _ = convert_to_video(os.path.join(
                    easyphoto_video_outpath_samples, "crop"), _new_outputs, actual_fps, prefix=prefix + "_crop", mode=save_as)
