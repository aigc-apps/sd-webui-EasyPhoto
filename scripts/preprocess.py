...
from easyphoto_process_utils import apply_mask_to_image, prepare_train_data_without_lightglue
...
def draw_points_on_image(img, points):
    ...
    for point in points:
        x, y = point
        cv2.circle(img_with_points, (int(x), int(y)), 20, (0, 0, 255), -1)
...
if __name__ == "__main__":
    ...
    if use_lightglue:
        ...
        print(f"Dynamic threshold is set at: {threshold}")
        ...
        # select salient object with no other env dependence
        prepare_train_data_without_lightglue(images_save_path, json_save_path, validation_prompt, inputs_dir, main_image_path, ref_image_path, use_mask)
